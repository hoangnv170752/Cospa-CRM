...
export async function webhookRoutes(fastify) {
    ...
    fastify.post('/webhooks/resend', {
        schema: {
            ...
            description: 'Receives webhook events from Resend (e.g. email.received for inbound emails). No authentication required — verify via Resend webhook secret.',
            body: {
                type: 'object',
                properties: {
                    type: { type: 'string' },
        ...
        if (event.type === 'email.received') {
            ...
            try {
                ...
                try {
                    ...
                    const emailContent = await fetch(`https://api.resend.com/emails/${emailData.email_id}/content`, {
                        headers: {
                ...
                console.log(`Inbound email received: ${inboundEmail.id} from ${emailData.from} subject="${emailData.subject}"`);
